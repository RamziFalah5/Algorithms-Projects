...
public class Dashboard {
	...
	public BorderPane ccc(Main main, BorderPane border) {
		...
		ImageView addImage = new ImageView("Add.png");
		...
		ImageView readFile = new ImageView("ReadFile.png");
		...
		ImageView ShowPotinalScheduleIameg = new ImageView("Arrow.png");
		...
		ImageView saveFileImage = new ImageView("SaveFile.png");
		...
		ImageView deletTaskIamge = new ImageView("Delete.png");
		...
		b1.setOnAction(x -> {
		    ...
		    if (name.isEmpty()) {
		        alert.AlertError("Invalid Name", "Name is Empty",
		    ...
		    if (prodText.isEmpty()) {
		        alert.AlertError("Invalid Productivity", "Productivity is Empty",
		                "Please enter productivity value.");
		        return;
		    }
		    ...
		    try {
		        ...
		        alert.AlertError("Invalid Time", "Wrong Format",
		                "Time must be a number.\nAllowed: 5 or 5.5");
		        return;
		    }

		    if (timeRequired <= 0) {
		        alert.AlertError("Invalid Time", "Non-positive Time",
		    ...
		    if (Math.abs(units - rounded) > 1e-9) {
		        alert.AlertError("Invalid Time", "Only .5 is Allowed",
		                "Time must be integer or end with .5 only.");
		        return;
		    }
		    ...
		    try {
		        ...
		        alert.AlertError("Invalid Productivity", "Wrong Format",
		                "Productivity must be an integer.");
		        return;
		    }

		    if (productivity <= 0) {
		        alert.AlertError("Invalid Productivity", "Non-positive Value",
		...
		readFileButton.setOnAction(e -> {
			...
			try (BufferedReader br = new BufferedReader(new FileReader(file))) {
			    ...
			    if (headerParts.length != 2) {
			        alert.AlertError("Invalid Header",
			                "First line must be: numberOfTasks,capacityTime",
			                "Example: 30,17:30");
			        return;
			    }
			    ...
			    alert.AlertError("Error Reading File",
